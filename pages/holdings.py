...
from utils.db import (get_client_advisors, get_advisor_clients, get_portfolios_for_ac,
                      ...
                      get_asset_price, submit_pending_asset, get_pending_assets_for_user, sb)
...
def _get_symbols(ac, sub):
    if ac == "Equity":
        ...
        return [(d["symbol"], d["name"], None, None) for d in data]
    if ac == "ETF":
        data = get_assets("ETF")
        return [(d["symbol"], d["name"], None, None) for d in data]
    if ac == "Mutual Fund":
        data = get_mutual_funds(sub_category=sub)
        return [(m["symbol"], m["name"], None, None) for m in data]
    if ac in ["Bond","Bank FD"]:
        data = get_fixed_income(ac)
        # Return (symbol, name, interest_rate, tenure_years)
        return [(f["symbol"], f["name"], f.get("interest_rate"), f.get("tenure_years")) for f in data]
    if ac == "Commodity":
        data = get_commodities()
        return [(c["symbol"], c["name"], None, None) for c in data]
    return []
...
def render():
    ...
    # ── ADD ASSET ─────────────────────────────────────────────────────────
    # Asset class and sub-category are OUTSIDE the form so they refresh immediately
    with tabs[2]:
        st.markdown("#### Select Asset")

        c1, c2 = st.columns(2)
        ac  = c1.selectbox("Asset Class",   list(ASSET_CLASSES.keys()), key="add_ac")
        sub = c2.selectbox("Sub-Category",  ASSET_CLASSES[ac]["sub"],   key="add_sub")

        sym_pairs = _get_symbols(ac, sub)
        utype     = ASSET_CLASSES[ac]["unit"]

        if not sym_pairs:
            st.warning("No assets found for this sub-category. Use Manual Entry below.")
        else:
            sym_opts  = [s for s,_,_,_ in sym_pairs]
            sym_names = {s:n for s,n,_,_ in sym_pairs}
            sym_rates = {s:r for s,_,r,_ in sym_pairs}
            sym_ten   = {s:t for s,_,_,t in sym_pairs}

            symbol = st.selectbox("Asset", sym_opts,
                                  format_func=lambda x: f"{x}  —  {sym_names.get(x,x)}",
                                  key="add_symbol")

            # Show live price or interest rate depending on asset class
            if ac in ("Bond","Bank FD"):
                ...
                if cur_p:
                    st.info(f"Closing price: ₹{indian_format(cur_p)}  ({chg:+.2f}%)")

            with st.form("add_holding_form"):
                # For FD/Bond show interest rate field that can be adjusted
                if ac in ("Bond","Bank FD"):
                    rate_val = sym_rates.get(symbol) or 0.0
                    interest_rate = st.number_input(
                        "Interest Rate (% p.a.) — adjustable",
                        value=float(rate_val), min_value=0.0, step=0.05, format="%.2f",
                        help="Prefilled from database. Adjust if your actual rate differs."
                    )
                    avg_cost_label = "Investment Amount (₹)" if utype == "amount" else "Price per Unit (₹)"
                else:
                    interest_rate = None
                    avg_cost_label = "Buy Price / NAV (₹)"
                ...
                avg_cost = st.number_input(avg_cost_label, min_value=0.01, step=1.0, format="%.2f")
                ...
                if st.form_submit_button("Add to Portfolio", use_container_width=True):
                    if qty <= 0 or avg_cost <= 0:
                        ...
                        # For FD/Bond, store custom interest rate in notes if different
                        note_str = notes
                        ...
                        st.success(f"Added {qty:g} {utype} of {symbol} at ₹{indian_format(avg_cost)}")
                        st.rerun()

    # ── MANUAL ENTRY ──────────────────────────────────────────────────────
    with tabs[3]:
        st.markdown("#### Add an Asset Not in the Database")
        st.caption("Submitted assets are verified within 24 hours. They appear with ⚠️ until verified.")
        with st.form("manual_holding"):
            ms  = st.text_input("Symbol / Ticker *", placeholder="e.g. NEWSTOCK")
            ...
            if st.form_submit_button("Submit for Verification", use_container_width=True):
                if not ms.strip() or not mn.strip():
                    ...
                    add_holding(pf_id, ms.strip().upper(), mac, msc, float(mq), mutype, mc, mnotes, is_manual=True)
    ...
    with tabs[4]:
        ...
        for pa in pending:
            ...
            with st.expander(f"{pa['symbol']}  ·  {pa['name']}  ·  <span style='color:{sc}'>{pa['status'].upper()}</span>"):
                st.markdown(f"**Class:** {pa.get('asset_class','')}  ·  **Submitted:** {fmt_date(str(pa.get('submitted_at',''))[:10])}<br>**Result:** {pa.get('verification_result','Awaiting review')}", unsafe_allow_html=True)
