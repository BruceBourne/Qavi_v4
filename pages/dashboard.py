...
from utils.db import (get_advisor_clients, get_client_advisors, get_portfolios_for_ac,
                      get_portfolio_holdings, get_asset_price,
                      get_pending_requests_for_advisor)
from utils.crypto import inr, fmt_date, title_case
...
def render():
    ...
    # Market status — correct 15:30 close
    open_badge = (
    ...
    if role == "advisor":
        ...
        if clients:
            ...
            for cl in clients[:5]:
                ...
                c1.markdown(f"<div style='font-weight:600'>{title_case(cl['client_name'])}</div><div style='font-size:.75rem;color:#8892AA'>{cl.get('client_email','')}</div>", unsafe_allow_html=True)
    ...
    # ── CLIENT ────────────────────────────────────────────────────────────
        ...
        if all_pfs:
            ...
            st.info("You haven't been linked to an advisor yet. You can still create private portfolios from the Portfolios page.")
