...
def _row(a, prices, key_sfx):
    ...
    hc[3].markdown(f"<div style='font-size:.8rem;color:#2ECC7A'>₹{indian_format(p.get('high',0))}</div><div style='font-size:.78rem;color:#FF5A5A'>₹{indian_format(p.get('low',0))}</div>", unsafe_allow_html=True)
...
def render():
    ...
    st.markdown('<div class="page-sub">NSE listed stocks by market cap</div>', unsafe_allow_html=True)
    ...
    search = st.text_input("🔍 Search equities", placeholder="Symbol or company name…", label_visibility="collapsed")
    prices = get_all_prices_map()
    assets = get_assets("Equity", search=search if search else None)

    by_sub = defaultdict(list)
    for a in assets: by_sub[a.get("sub_class","Other")].append(a)
    ...
    for sub in ["Large Cap","Mid Cap","Small Cap"]:
        items = by_sub.get(sub,[])
        if not items: continue
        with st.expander(f"**{sub}** — {len(items)} stocks", expanded=(sub=="Large Cap")):
            for a in items: _row(a, prices, sub)
