...
def _row(a, prices, key_sfx):
    ...
    sign = "▲" if chg >= 0 else "▼"
    hc   = st.columns([2.5, 1.2, 1.2, 1.5, 1.8, 0.6])
    hc[0].markdown(
        f"<div style='font-weight:600;font-size:.87rem'>{a['symbol']}</div>"
        f"<div style='font-size:.7rem;color:#8892AA'>{a['name']}</div>",
        unsafe_allow_html=True)
    hc[1].markdown(f"<div style='font-size:.83rem'>₹{indian_format(p.get('open',0))}</div>", unsafe_allow_html=True)
    hc[2].markdown(
        ...
        unsafe_allow_html=True)
    hc[3].markdown(f"<div style='font-size:.95rem;font-weight:700'>₹{indian_format(cl)}</div>", unsafe_allow_html=True)
    hc[4].markdown(f"<div style='color:{cc};font-weight:700;font-size:.9rem'>{sign} {abs(chg):.2f}%</div>", unsafe_allow_html=True)
    if hc[5].button("→", key=f"det_{a['symbol']}_{key_sfx}"):
...
def render():
    ...
    search = st.text_input("🔍 Search", placeholder="Symbol or company name…",
    ...
    # Group by sub_class; anything empty → "Other"
    by_sub = defaultdict(list)
    for a in assets:
        sub = a.get("sub_class","") or "Other"
        by_sub[sub].append(a)

    # Show all sub-categories (not just hardcoded 3)
    ...
    # Header row
    hdr = st.columns([2.5, 1.2, 1.2, 1.5, 1.8, 0.6])
    for col, lbl in zip(hdr, ["Symbol", "Open", "High / Low", "LTP", "Change", ""]):
