...
def _collect_portfolio_data(pf_ids, pmap, assets_map, period_days,
    ...
    """
    if date_from is None: date_from = date.today() - timedelta(days=period_days)
    if date_to   is None: date_to   = date.today()
    all_holdings = []
    for pf_id in pf_ids:
    ...
    for h in sorted(holdings_detail, key=lambda x: -x["current"])[:15]:
        ...
        hist = [r for r in hist if str(date_from) <= r.get("price_date","") <= str(date_to)]
...
def build_report_html(
    ...
    include_projections, portfolio_names, period_days=365
):
    """
...
<div class="page">
  ...
  <div class="pgnum">Page 1 of {TOTAL}</div>
    ...
    ann = _annualised(data["total_pnl_pct"], max(period_days, 1))
    pages.append(f"""
<div class="page">
...
<div class="page">
  ...
  {''.join(_bar(k, v['cur'], data['total_cur'], CLASS_COLORS.get(k.split()[0] if k else "","#94a3b8"))
    ...
    best_h = max(data["holdings"], key=lambda x: x["pnl_pct"]) if data["holdings"] else None
    best_sym = best_h["symbol"] if best_h else "—"
    best_pct = f"{best_h['pnl_pct']:+.1f}%" if best_h else ""
    ann_str  = f"{ann:+.2f}%"
    pages.append(f"""
<div class="page">
  ...
  <div class="metrics">
    ...
    <div class="metric">
      ...
      <div class="val {'up' if ann>=0 else 'dn'}">{ann_str}</div>
    </div>
    <div class="metric">
      <div class="lbl">Best Performer</div>
      <div class="val up" style="font-size:.9rem">{best_sym}</div>
      <div class="sub up">{best_pct}</div>
    </div>
  </div>
  ...
  <table>
    ...
    if include_risk:
        ...
        vol_lbl  = "Low" if data["volatility"] < 10 else ("Moderate" if data["volatility"] < 20 else "High")
        vol_str  = f"{data['volatility']:.2f}%"
        mdd_str  = f"{data['max_dd']:.2f}%"
        pages.append(f"""
<div class="page">
  ...
  <div class="metrics">
    <div class="metric">
      ...
      <div class="val {sh_color}">{sh_str}</div>
      <div class="sub neu">{sh_label}</div>
    </div>
    <div class="metric">
      <div class="lbl">Annualised Volatility</div>
      <div class="val">{vol_str}</div>
      <div class="sub neu">{vol_lbl}</div>
    </div>
    <div class="metric">
      <div class="lbl">Max Drawdown (1Y)</div>
      <div class="val dn">{mdd_str}</div>
    </div>
    <div class="metric">
      <div class="lbl">Diversification</div>
      <div class="val">{data['div_score']}/100</div>
  ...
  <div class="sec">Risk by Asset Class</div>
        risk_rows = ""
        for k,v in sorted(data["by_class"].items(), key=lambda x:-x[1]["cur"]):
            wt   = f"{v['cur']/data['total_cur']*100:.1f}%" if data["total_cur"] else "0%"
            rlvl = "High" if k in ("Equity","Crypto") else ("Moderate" if k in ("Mutual Fund","ETF","Commodity") else "Low")
            rcls = "dn" if rlvl=="High" else ("neu" if rlvl=="Moderate" else "up")
            note = ("Market risk, price volatility" if k=="Equity"
            ...
            risk_rows += f'''<tr><td><b>{k}</b></td><td>{wt}</td>
              <td class="{rcls}">{rlvl}</td>
              <td style="color:#64748b;font-size:.75rem">{note}</td></tr>'''
          <table>
...
def render():
    ...
    date_from  = pc1.date_input("From", value=today - timedelta(days=365),
                                 max_value=today - timedelta(days=1))
    date_to    = pc2.date_input("To",   value=today, max_value=today)
    # Quick presets
    preset = pc3.selectbox("Quick preset", [
        "Custom", "1 Month", "3 Months", "6 Months",
        "1 Year", "3 Years", "5 Years", "All Time (10Y)"
    ], index=4)
    if preset != "Custom":
        preset_days = {
        ...
        date_from = today - timedelta(days=preset_days)
        date_to   = today
    ...
    period_label = (preset if preset != "Custom"
    ...
    if st.button("📄 Generate Report", use_container_width=True, type="primary"):
        with st.spinner("Building report — fetching data and computing metrics…"):
            try:
                data = _collect_portfolio_data(sel_pfs, pmap, assets_map, period_days, date_from, date_to)
            ...
            html       = build_report_html(
                ...
                include_risk, include_scenarios, include_stress,
                include_projections, pf_names, period_days
            )
